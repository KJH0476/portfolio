...
Slack 기반 TIL 챌린지에서는 새 답글이 등록될 때, 해당 글에 참여한 일부 멤버만 알림을 받고, 챌린지에 참여하지 않은 멤버들은 새로운 학습 내용이 게시된 사실을 알 수 없었습니다. 이로 인해 ‘서로의 학습 내용을 공유한다’는 챌린지의 본래 취지를 살리지 못한다고 생각했습니다. </br>
또한, 기존에는 매니저님이 매주 수동으로 TIL N월 N주차 글을 작성하고, 참여 현황과 좋아요(👍) 수를 직접 집계하는 방식이 비효율적이라고 판단했습니다. 이러한 문제를 개선하기 위해 본 프로젝트를 시작하게 되었습니다.

#### 목표

- Slack 기반 **TIL 챌린지의 운영 과정을 자동화**
- 참여 현황 및 성과를 명확히 파악할 수 있도록 하는 챗봇 및 대시보드를 제공
...
1. **TIL 챌린지 메세지 및 참여 알림 메시지 게시 자동화**
...
4. **참여 현황 시각화 대시보드 제공**  
   ...
   - 주차별 참여 인원 및 좋아요 수를 한눈에 확인 가능
	![[dashboard.png]]
5. **운영을 위한 Slack 개인 DM 좋아요 및 에러 알림**  
   - 좋아요 변동 시 본인 DM으로 상태 메시지 전송
   - 시스템 에러 발생 시 에러 알림 메시지 전송
